 ...
 * shell — line numbers, line gutters, annotation badges — and the caller
 * can pass already-rendered HTML via `htmlContent` for highlighting.
 *
...
export interface RegoSyntaxProps {
  ...
  code?: string;
  /** Pre-highlighted HTML (e.g. from Shiki). Takes precedence over `code`. */
  htmlContent?: string;
...
function naiveColorize(line: string): ReactNode {
  ...
  return (
    <>
      {before}
      <span style={{ color: "var(--egide-color-text-muted)", fontStyle: "italic" }}>
        {m[1]}
      </span>
    </>
  );
...
export function RegoSyntax({
  ...
  return (
    <figure className={className} style={containerStyle}>
      ...
      {htmlContent ? (
        <div
          style={{ padding: "var(--egide-space-3)", lineHeight: 1.5 }}
          // biome-ignore lint/security/noDangerouslySetInnerHtml: caller is responsible for sanitizing pre-rendered HTML (Shiki output is safe)
          dangerouslySetInnerHTML={{ __html: htmlContent }}
        />
      ) : (
        <pre style={{ margin: 0, padding: "var(--egide-space-3) 0", overflowX: "auto" }}>
          {lines.map((line, i) => {
            const lineNum = i + 1;
            const inHighlight =
              !!highlightRange && lineNum >= highlightRange[0] && lineNum <= highlightRange[1];
            ...
            return (
              <span
                ...
                style={{
                  ...
                  background: inHighlight ? "var(--egide-color-accent-muted)" : "transparent",
              ...
              >
                ...
                {annos && annos.length > 0 && (
                  <span style={{ display: "inline-flex", gap: "var(--egide-space-1)" }}>
                    {annos.map((a) => {
